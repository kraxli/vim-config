...
inoremap <expr><silent> <Tab>   pumvisible() ? "\<Down>" : (complete#is_whitespace() ? "\<Tab>" : asyncomplete#force_refresh())
inoremap <expr><silent> <S-Tab> pumvisible() ? "\<Up>"   : (complete#is_whitespace() ? "\<C-h>" : asyncomplete#force_refresh())
...
" kraxli: moved autolaod/complete
